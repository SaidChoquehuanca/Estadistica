...
```{r}
barplot(DF2017$Discapacidad, col = "lightblue",main = "Población de Ninis por región", las=2, names.arg=(DF2017$Departamento),las=2, ylim=c(0,66000))
```
...
```{r}
barplot(DF2017$Gastos_educacion, col = "pink",main = "Gastos en educación por región", las=2, names.arg=(DF2017$Departamento),las=2)
```
```{r}
...
H0: El porcentaje de embarazos adolescentes representa menos del 50% de población de ninis.
H1:El porcentaje de embarazos adolescentes representa más del 50% de ninis en Lima.

```{r}
ME_Ad<-0.5*mean(DF2017$Numero_ninis/DF2017$`Poblacion_14-19`,na.rm=TRUE)
```
```{r}
TEST2<-t.test(DF2017$`%Embarazo_ado`,mu=ME_Ad,conf.level = 0.95,alternative = "greater")

pvalor2<-TEST2$p.value
pvalor2

rechazo<-function(p)
  {
  return(pbinom(15,size=100,prob=p,lower.tail = FALSE))
  }

X<-seq(0,1,by=0.01)
plot(X,rechazo(X),type="l",ylab="Probabilidad de nini",axes=FALSE,ylim=c(-0.1,1),xlab="Pvalor", main="50% de embarazos adolescentes representan población Nini")
abline(v=0.05,col="red")
abline(v=pvalor2,col="blue")
axis(1)
axis(2)
arrows(0,-0.05,0.02,-0.05,code=3,length=0.1)
text(0.5,-0.02,expression(H[0]),pos=1,adj=c(0,1))
arrows(0.02,-0.05,1,-0.05,code=3,length=0.1)
text(0,-0.02,expression(H[1]),pos=1,adj=c(0,1))
```
Debido que p-value es menor al error, se rechaza H0 afirmando que el porcentaje de embarazadas representa más del 50% de población nini de 14 a 19 años.
...
```{r}
PobN<-0.5*mean(DF2017$Numero_ninis,na.rm=TRUE)
```
...
```{r}
TEST3<-t.test(DF2017$Discapacidad,mu=8811.82,conf.level = 0.95,alternative = "greater")
pvalor3<-TEST3$p.value
```

```{r}
plot(X,rechazo(X),type="l",ylab="Probabilidad de nn",axes=FALSE,ylim=c(-0.1,1),xlab="Pvalor", main="Prueba de hipótesis: Encontrar un NiNi")
abline(v=0.05,col="red")
abline(v=pvalor3,col="blue")
axis(1)
axis(2)
arrows(0,-0.05,0.02,-0.05,code=3,length=0.1)
text(0.5,-0.02,expression(H[0]),pos=1,adj=c(0,1))
...
```
  ...
  De acuerdo al p-value, este es mayor al alfa, no se rechaza H0, es decir que el número de personas discapacitadas no bordea el 50% de población nini. 
...
title(main = " Ninis y Discapacidad por región ")
abline(a=0,b=0)
...
16) ¿En qué medida una persona discapacitada forma parte de la población nini?

Una persona NINI es aquella que no estudia ni trabaja, sin embargo, existen múltiples factores tanto internos como externos que explican el porqué de su existencia. Por otro lado, de acuerdo con Casas (2018) informa que existe un crecimiento de personas discapacitadas en el país, pero que esto no se relaciona con la contratación de personal con estas características en las empresas públicas y privadas del Perú. Asimismo, según el estudio del CONADIS Perú (2015) solo el 38.5% de esta población accede a la educación ya que cuenta con múltiples factores que limitan su asistencia tanto escolar como universitaria: Los colegios especiales suelen estar en zonas urbanas, la mayoría de centros educativos públicas no cuentan con áreas que impulsen la aceptación de población discapacitada, etc. De esta manera, podemos afirmar que la población discapacitada está más propensa a formar parte de la población nini, ya que presentan limitaciones externas a nivel social en el sector laboral y educativo.  

17) ¿En qué medida los embarazos adolescentes origina una alza de población nini?

Según un informe del INEI (2018) afirma que aproximadamente nueve de cada diez madres abandonaron la escuela. Esto explica por qué existe una relación proporcional entre el porcentaje de embarazos adolescentes con el de ninis por cada región. Por otro lado, Mendoza y Subiría (2013) "un embarazo no planificado disminuye las posibilidades de continuar una transición hacia la vida adulta, en especial, si viven en condiciones de pobreza o pobreza extrema". De esta manera, vemos que este puede ser considerado un factor importante para el aumento de población nini, ya que un gran número de chicas adolescentes desertan en orden de realizar otras actividades para mantener a sus hijos. Sin embargo, debemos tomar en cuenta que las embarazadas adolescentes tienen más oportunidades de trabajo en comparación a las personas discapacitadas, ya que cuentan con ventajas a nivel físico y social.
...
Casas, A.(2018) ¿Cuál es la situación laboral de las personas con discapacidad en el Perú?. Recuperado el 04 de julio, 2019, de https://elcomercio.pe/peru/situacion-personas-discapacidad-peru-noticia-568171?foto=7 

CONADIS PERÚ (2018). Cifras sobre Educación y Discapacidad en el Perú. Recuperado el 04 de julio, 2019, de http://www4.congreso.gob.pe/comisiones/2002/discapacidad/educacion_inclusiva/cifras-discapacidad.pdf 
