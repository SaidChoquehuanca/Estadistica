...
```{r}
barplot(DF2017$Gastos_educacion, col = "pink",main = "Población de Ninis por región", las=2, names.arg=(DF2017$Departamento),las=2)
```
```{r}
...
plot(DF2017$Numero_ninis/1000 ,col="red", main = "Población de Ninis por Departamento" , xlab = "Departamento por numero 1-25" , ylab = "Ninis en Miles ")
...
plot(DF2017$Discapacidad/1000 ,col="blue", main = "Discapacitados por Departamento" , xlab = "Departamento por numero 1-25" , ylab = "Discapacitados en Miles")
...
```{r}
barplot(DF2017$`%Embarazo_ado` , xlab = "Departamentos del 1-25" , ylab = "(%)Porcentaje de Embarazos" , main = "Porcentaje de embarazos adolescentes en el Perú") 
```
...
PREGUNTAS PREDICTIVAS:
 ...
 16) ¿Es posible estabelcer una relación entre la delincuencia respecto el número de ninis en un departamento en el año 2017?
 
```{r}
plot(DF2017$Numero_ninis,DF2017$Discapacidad, pch=2, xlab="Población de Ninis", ylab ="Discapacidad", col="grey",lwd=3, main="Población NiNi vs. discapacidad")
ml<-lm(D~n)
summary(ml)
abline(ml,col="blue")
```
Observamos que existe una relación entre nuestras variables que puede ser explicada con una recta, y vemos que esta es significativa porque el p-value es muy pequeño. Sin embargo, el valor de R cuadrado, es bajo, significa que nuestra grafica no explica las varianzas de los valores observados. Esto se puede deber a que existen otros varios factores en los departametos que alteran el numero de personas discapacitadas los cuales no están siendo considerados en la regresión. NUestra aproximacion puede ser utilizada para darnos una idea de como será la deliuencia respecto al número de ninis, pero tendríamos que tener en cuenta que este resultado no va a ser preciso.
